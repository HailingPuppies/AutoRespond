...
def get_message(user='lifeupdate.msg@gmail.com', password='rvqeiqxcoftltxgg', most_recent_uid=None):
    ...
    if most_recent_uid != most_recent.decode('utf-8'):
        ...
        # print('Already read')
        return most_recent.decode('utf-8') + 1
    else:
...
def auto_reply():
    #----------------------CHECK IF ANY MESSAGES HAVE BEEN READ--------------------
    list = os.listdir('downloaded_messages')
    number_files = len(list)
    # print(f'files: {number_files}')
    if number_files == 0:
        print('No new messages')
        return
    most_recent_uid = get_most_recent_uid()
    print(f'most recent uid: {most_recent_uid}')

    #this goes at the end:
    fname = os.listdir("downloaded_messages")[0]
    os.rename(f'downloaded_messages/{fname}',f'archived_messages/{fname}')



if __name__ == '__main__':
    # recent_uid = get_most_recent_uid()
    # print(f'recent_uid {recent_uid}')
    # recent_uid = get_message(most_recent_uid=recent_uid)
    # print(f'recent_uid post get message {recent_uid}')
